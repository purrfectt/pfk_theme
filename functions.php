...
// Get PFK Core Up & Running!
require_once('library/pfk.php');            // core functions (don't remove)
...
<?php the_post_thumbnail( 'pfk-thumb-300' ); ?>
for the 600 x 100 image:
<?php the_post_thumbnail( 'pfk-thumb-600' ); ?>
...
function pfk_register_sidebars() {
...
// Comment Layout
function pfk_comments($comment, $args, $depth) {
		...
		<article id="comment-<?php comment_ID(); ?>" class="clearfix">
			<div class="comment-author vcard row-fluid clearfix">
				...
				<div class="span9 comment-text">
					<?php printf(__('<h4>%s</h4>','pfk'), get_comment_author_link()) ?>
					<?php edit_comment_link(__('Edit','pfk'),'<span class="edit-comment btn btn-small btn-info"><i class="icon-white icon-pencil"></i>','</span>') ?>
                    
                    <?php if ($comment->comment_approved == '0') : ?>
       					<div class="alert-message success">
          				<p><?php _e('Your comment is awaiting moderation.','pfk') ?></p>
          				</div>
					<?php endif; ?>
...
// Search Form
function pfk_wpsearch($form) {
    $form = '<form role="search" method="get" id="searchform" action="' . home_url( '/' ) . '" >
    <label class="screen-reader-text" for="s">' . __('Search for:', 'pfk') . '</label>
    <input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="Search the Site..." />
    <input type="submit" id="searchsubmit" value="'. esc_attr__('Search','pfk') .'" />
    </form>';
    return $form;
} // don't remove this bracket!
...
function custom_password_form() {
	...
	' . __( "<p>This post is password protected. To view it please enter your password below:</p>" ,'pfk') . '
	<label for="' . $label . '">' . __( "Password:" ,'pfk') . ' </label><div class="input-append"><input name="post_password" id="' . $label . '" type="password" size="20" /><input type="submit" name="Submit" class="btn btn-primary" value="' . esc_attr__( "Submit",'pfk' ) . '" /></div>
	</form></div>
	';
	return $o;
}
